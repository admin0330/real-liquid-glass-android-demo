...
- 将 Browse 或 Radio 页面实现为外部内容入口。

Browse 使用本地元数据生成浏览分区。Radio 使用本地曲库、收藏和播放历史生成智能队列。二者的输入、输出均为设备内数据。

### 2.3 应用更新是独立例外

应用更新可以访问预配置的 HTTPS 更新清单和 APK，但它不属于音乐域。更新器必须使用独立包、独立接口和受限网络客户端，不得被 Repository、UseCase、播放器或任何音乐 feature 调用。
...
Browse 和 Radio 的规则必须落在 domain/usecase 中，确保可测试、可解释且不依赖 Composable 的生命周期。
...
- 迷你播放器放在顶层 scaffold，与五栏导航共享明确的 content inset；
...
五个顶层目的地固定为 Listen Now、Browse、Radio、Library 和 Search。设置通过 Library/个人入口或顶层动作进入，不占用第六个主 Tab。
...
依赖结构必须保证删除整个 `update` 包后，本地曲库、播放、歌词、Browse、Radio 和设置仍可正常工作。
...
- domain UseCase：排序、分组、搜索、收藏、播放历史、本地 Radio 生成；
